...
{
    $stmt=$mysqli->prepare("insert into new_data (name,Email,Password,confirm_Password,RoomNum,Ext,image) values(?,?,?,?,?,?,?)");
    $stmt->bind_param("sssssss",$Name,$Email,$password,$cpassword,$Rm,$Text,$imgNewName);
    $stmt->execute();
    echo"<h1 style='text-align: center; color:Green; margin-top:300px'>Registration Successful</h1>";
    $stmt->close();
    $mysqli->close();
